...
Клас Train повинен містити:
	...
	 методи:
		...
		o номер вагону в якому їхала найменша кількість пасажирів
		o номер вагону в якому їхала найбільша кількість пасажирів.*/
...
int main()
{
	...
	int passengers;  // для вводу пасажирів
	...
	for (int j = 0; j < x; j++)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			cout << "Enter passengers,pls: \n";
			cout << "Note: if number was be inccorect I choose random number!\n";
			cout << "Answer: ";
			cin >> passengers;

			tempCar[i].SetTypeCar(type);
			tempCar[i].SetPassengers(passengers);
		}
